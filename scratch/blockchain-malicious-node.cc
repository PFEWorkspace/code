...
NS_LOG_COMPONENT_DEFINE ("BlockchainMaliciousNode");

NS_OBJECT_ENSURE_REGISTERED (BlockchainMaliciousNode);

TypeId
BlockchainMaliciousNode::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BlockchainMaliciousNode")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<BlockchainMaliciousNode> ()
    .AddAttribute ("Local",
                   "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&BlockchainMaliciousNode::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Protocol",
                   "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&BlockchainMaliciousNode::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("NumberOfMiners",
				   "The number of miners",
                   UintegerValue (16),
                   MakeUintegerAccessor (&BlockchainMaliciousNode::m_noMiners),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockSize",
				   "The fixed size of the block",
                   UintegerValue (0),
                   MakeUintegerAccessor (&BlockchainMaliciousNode::m_fixedBlockSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockIntervalGeneration",
                   "The fixed time to wait between two consecutive block generations",
                   DoubleValue (0),
                   MakeDoubleAccessor (&BlockchainMaliciousNode::m_fixedBlockTimeGeneration),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("InvTimeoutMinutes",
				   "The timeout of inv messages in minutes",
                   TimeValue (Minutes (20)),
                   MakeTimeAccessor (&BlockchainMaliciousNode::m_invTimeoutMinutes),
                   MakeTimeChecker())
    .AddAttribute ("HashRate",
				   "The hash rate of the selfish miner",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&BlockchainMaliciousNode::m_hashRate),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("BlockGenBinSize",
				   "The block generation bin size",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&BlockchainMaliciousNode::m_blockGenBinSize),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("BlockGenParameter",
				   "The block generation distribution parameter",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&BlockchainMaliciousNode::m_blockGenParameter),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("AverageBlockGenIntervalSeconds",
				   "The average block generation interval we aim at (in seconds)",
                   DoubleValue (10*60),
                   MakeDoubleAccessor (&BlockchainMaliciousNode::m_averageBlockGenIntervalSeconds),
                   MakeDoubleChecker<double> ())
    .AddTraceSource ("Rx",
                     "A packet has been received",
                     MakeTraceSourceAccessor (&BlockchainMaliciousNode::m_rxTrace),
...
BlockchainMaliciousNode::BlockchainMaliciousNode () : BlockchainValidator(), m_attackFinished(false), m_la(0), m_lh(0), m_forkType(IRRELEVANT)
{
...
BlockchainMaliciousNode::~BlockchainMaliciousNode(void)
{
  NS_LOG_FUNCTION (this);
}
...
void
BlockchainMaliciousNode::StartApplication ()    // Called at time specified by Start
{
...
void
BlockchainMaliciousNode::StopApplication ()
{
...
void
BlockchainMaliciousNode::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
  BlockchainValidator::DoDispose ();

}

void
BlockchainMaliciousNode::MineBlock (void)
{
  NS_LOG_FUNCTION (this);
...
void
BlockchainMaliciousNode::ReceivedHigherBlock(const Block &newBlock)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_WARN("Blockchain malicious node "<< GetNode ()->GetId () << " added a new block in the m_blockchain with higher height: " << newBlock);
...
void
BlockchainMaliciousNode::ReceiveBlock(const Block &newBlock)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO ("BlockchainMaliciousNode ReceiveBlock: At time " << Simulator::Now ().GetSeconds ()
                << "s blockchain node " << GetNode ()->GetId () << " received " << newBlock);
  ...
  {
    NS_LOG_INFO ("BlockchainMaliciousNode ReceiveBlock: Blockchain node " << GetNode ()->GetId () << " has already added this block in the m_blockchain: " << newBlock);
  ...
  {
    NS_LOG_INFO ("BlockchainMaliciousNode ReceiveBlock: Blockchain node " << GetNode ()->GetId () << " has NOT added this block in the m_blockchain: " << newBlock);
...
void
BlockchainMaliciousNode::ReleaseChain(std::vector<Block> blocks)
{
  NS_LOG_FUNCTION (this);
...
/*                 std::cout << "Node " << GetNode()->GetId() << "-" << InetSocketAddress::ConvertFrom(from).GetIpv4 ()
        ...
        Simulator::Schedule (Seconds(eventTime), &BlockchainMaliciousNode::SendBlock, this, packet, m_peersSockets[*i]);
        Simulator::Schedule (Seconds(eventTime + sendTime), &BlockchainMaliciousNode::RemoveSendTime, this);
...
/* 				std::cout << "Node " << GetNode()->GetId() << "-" << *i
          ...
          std::string packet = blockInfo.GetString();
          Simulator::Schedule (Seconds(sendTime), &BlockchainMaliciousNode::SendBlock, this, packet, m_peersSockets[*i]);
          Simulator::Schedule (Seconds(eventTime + sendTime), &BlockchainMaliciousNode::RemoveCompressedBlockSendTime, this);
        ...
        else
        {
          ...
          NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds ()
                       << "s blockchain validator " << GetNode ()->GetId ()
                       << " sent a packet " << invInfo.GetString()
                       << " to " << *i);
        }
        break;
...
enum Action
BlockchainMaliciousNode::ReadActionMatrix(enum ForkType f, int la, int lh)
{
  NS_LOG_FUNCTION (this);
