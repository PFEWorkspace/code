...
NS_LOG_COMPONENT_DEFINE ("BlockchainSimpleAttacker");

NS_OBJECT_ENSURE_REGISTERED (BlockchainSimpleAttacker);

TypeId
BlockchainSimpleAttacker::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BlockchainSimpleAttacker")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<BlockchainSimpleAttacker> ()
    .AddAttribute ("Local",
                   "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&BlockchainSimpleAttacker::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Protocol",
                   "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&BlockchainSimpleAttacker::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("NumberOfMiners",
				   "The number of miners",
                   UintegerValue (16),
                   MakeUintegerAccessor (&BlockchainSimpleAttacker::m_noMiners),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockSize",
				   "The fixed size of the block",
                   UintegerValue (0),
                   MakeUintegerAccessor (&BlockchainSimpleAttacker::m_fixedBlockSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockIntervalGeneration",
                   "The fixed time to wait between two consecutive block generations",
                   DoubleValue (0),
                   MakeDoubleAccessor (&BlockchainSimpleAttacker::m_fixedBlockTimeGeneration),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("InvTimeoutMinutes",
				   "The timeout of inv messages in minutes",
                   TimeValue (Minutes (20)),
                   MakeTimeAccessor (&BlockchainSimpleAttacker::m_invTimeoutMinutes),
                   MakeTimeChecker())
    .AddAttribute ("HashRate",
				   "The hash rate of the simple attacker",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&BlockchainSimpleAttacker::m_hashRate),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("BlockGenBinSize",
				   "The block generation bin size",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&BlockchainSimpleAttacker::m_blockGenBinSize),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("BlockGenParameter",
				   "The block generation distribution parameter",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&BlockchainSimpleAttacker::m_blockGenParameter),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("AverageBlockGenIntervalSeconds",
				   "The average block generation interval we aim at (in seconds)",
                   DoubleValue (10*60),
                   MakeDoubleAccessor (&BlockchainSimpleAttacker::m_averageBlockGenIntervalSeconds),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SecureBlocks",
				   "The number of blocks required for the secure confirmation of the transactions",
                   UintegerValue (6),
                   MakeUintegerAccessor (&BlockchainSimpleAttacker::m_secureBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("AdvertiseBlocks",
				   "Choose whether the attacker will advertise his generated blocks",
                   UintegerValue (0),
                   MakeUintegerAccessor (&BlockchainSimpleAttacker::m_advertiseBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddTraceSource ("Rx",
                     "A packet has been received",
                     MakeTraceSourceAccessor (&BlockchainSimpleAttacker::m_rxTrace),
...
BlockchainSimpleAttacker::BlockchainSimpleAttacker () : BlockchainValidator(), m_attackFinished(false)
{
  NS_LOG_FUNCTION (this);
}


BlockchainSimpleAttacker::~BlockchainSimpleAttacker(void)
{
  NS_LOG_FUNCTION (this);
}
...
void
BlockchainSimpleAttacker::StartApplication ()    // Called at time specified by Start
{
...
void
BlockchainSimpleAttacker::StopApplication ()
{
...
void
BlockchainSimpleAttacker::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
void
BlockchainSimpleAttacker::MineBlock (void)
{
  NS_LOG_FUNCTION (this);
...
/* 	  //Send large packet
  ...
  NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds ()
               << "s blockchain simple attacker " << GetNode ()->GetId ()
...
void
BlockchainSimpleAttacker::ReceivedHigherBlock(const Block &newBlock)
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO("Blockchain simple attacker "<< GetNode ()->GetId () << " added a new block in the m_blockchain with higher height: " << newBlock);
