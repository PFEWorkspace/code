...
NS_LOG_COMPONENT_DEFINE ("BlockchainMaliciousNodeTrials");

NS_OBJECT_ENSURE_REGISTERED (BlockchainMaliciousNodeTrials);

TypeId
BlockchainMaliciousNodeTrials::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::BlockchainMaliciousNodeTrials")
    .SetParent<Application> ()
    .SetGroupName("Applications")
    .AddConstructor<BlockchainMaliciousNodeTrials> ()
    .AddAttribute ("Local",
                   "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&BlockchainMaliciousNodeTrials::m_local),
                   MakeAddressChecker ())
    .AddAttribute ("Protocol",
                   "The type id of the protocol to use for the rx socket.",
                   TypeIdValue (UdpSocketFactory::GetTypeId ()),
                   MakeTypeIdAccessor (&BlockchainMaliciousNodeTrials::m_tid),
                   MakeTypeIdChecker ())
    .AddAttribute ("NumberOfMiners",
				   "The number of miners",
                   UintegerValue (16),
                   MakeUintegerAccessor (&BlockchainMaliciousNodeTrials::m_noMiners),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockSize",
				   "The fixed size of the block",
                   UintegerValue (0),
                   MakeUintegerAccessor (&BlockchainMaliciousNodeTrials::m_fixedBlockSize),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("FixedBlockIntervalGeneration",
                   "The fixed time to wait between two consecutive block generations",
                   DoubleValue (0),
                   MakeDoubleAccessor (&BlockchainMaliciousNodeTrials::m_fixedBlockTimeGeneration),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("InvTimeoutMinutes",
				   "The timeout of inv messages in minutes",
                   TimeValue (Minutes (20)),
                   MakeTimeAccessor (&BlockchainMaliciousNodeTrials::m_invTimeoutMinutes),
                   MakeTimeChecker())
    .AddAttribute ("HashRate",
				   "The hash rate of the Malicious Node",
                   DoubleValue (0.2),
                   MakeDoubleAccessor (&BlockchainMaliciousNodeTrials::m_hashRate),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("BlockGenBinSize",
				   "The block generation bin size",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&BlockchainMaliciousNodeTrials::m_blockGenBinSize),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("BlockGenParameter",
				   "The block generation distribution parameter",
                   DoubleValue (-1),
                   MakeDoubleAccessor (&BlockchainMaliciousNodeTrials::m_blockGenParameter),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("AverageBlockGenIntervalSeconds",
				   "The average block generation interval we aim at (in seconds)",
                   DoubleValue (10*60),
                   MakeDoubleAccessor (&BlockchainMaliciousNodeTrials::m_averageBlockGenIntervalSeconds),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SecureBlocks",
				   "The number of blocks required for the secure confirmation of the transactions",
                   UintegerValue (6),
                   MakeUintegerAccessor (&BlockchainMaliciousNodeTrials::m_secureBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("AdvertiseBlocks",
				   "Choose whether the attacker will advertise his generated blocks",
                   UintegerValue (0),
                   MakeUintegerAccessor (&BlockchainMaliciousNodeTrials::m_advertiseBlocks),
                   MakeUintegerChecker<uint32_t> ())
    .AddTraceSource ("Rx",
                     "A packet has been received",
                     MakeTraceSourceAccessor (&BlockchainMaliciousNodeTrials::m_rxTrace),
...
BlockchainMaliciousNodeTrials::BlockchainMaliciousNodeTrials () : BlockchainValidator(), m_attackFinished(false), m_winningStreak(0), m_trials(1)
{
  NS_LOG_FUNCTION (this);
}


BlockchainMaliciousNodeTrials::~BlockchainMaliciousNodeTrials(void)
{
  NS_LOG_FUNCTION (this);
}
...
void
BlockchainMaliciousNodeTrials::StartApplication ()    // Called at time specified by Start
{
  BlockchainNode::StartApplication ();
  NS_LOG_WARN ("Malicious Node " << GetNode()->GetId() << " m_realAverageBlockGenIntervalSeconds = " << m_realAverageBlockGenIntervalSeconds << "s");
  NS_LOG_WARN ("Malicious Node " << GetNode()->GetId() << " m_averageBlockGenIntervalSeconds = " << m_averageBlockGenIntervalSeconds << "s");
  NS_LOG_WARN ("Malicious Node " << GetNode()->GetId() << " m_fixedBlockTimeGeneration = " << m_fixedBlockTimeGeneration << "s");
  NS_LOG_WARN ("Malicious Node " << GetNode()->GetId() << " m_secureBlocks = " << m_secureBlocks);
  NS_LOG_WARN ("Malicious Node " << GetNode()->GetId() << " m_hashRate = " << m_hashRate << " " << m_advertiseBlocks);
  NS_LOG_WARN ("Malicious Node " << GetNode()->GetId() << " m_advertiseBlocks = " << m_advertiseBlocks);
...
void
BlockchainMaliciousNodeTrials::StopApplication ()
{
  BlockchainNode::StopApplication ();
  Simulator::Cancel (m_nextMiningEvent);

  NS_LOG_WARN ("The Malicious Node " << GetNode ()->GetId () << " with hash rate = " << m_hashRate << " generated " << m_minerGeneratedBlocks
...
void
BlockchainMaliciousNodeTrials::DoDispose (void)
{
  NS_LOG_FUNCTION (this);
...
void
BlockchainMaliciousNodeTrials::MineBlock (void)
{
  NS_LOG_FUNCTION (this);
...
/* 	  //Send large packet
  ...
  NS_LOG_WARN ("At time " << Simulator::Now ().GetSeconds ()
               << "s blockchain malicious node " << GetNode ()->GetId ()
...
void
BlockchainMaliciousNodeTrials::ReceivedHigherBlock(const Block &newBlock)
{
  NS_LOG_FUNCTION (this);
